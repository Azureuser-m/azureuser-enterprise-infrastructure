...
$ScriptLog    = Join-Path $LogRoot 'wazuh-server-startup.log'
$MsiLog       = Join-Path $LogRoot 'wazuh-server-msi.log'
...
try {
    Write-Log "=== Wazuh server startup script started ==="
    ...
    Write-Log "=== Wazuh server startup script completed successfully ==="
    exit 0
}
catch {
